...
#include <tinyply/example-utils.hpp>
...
// from the tinyply example given here: https://github.com/ddiakopoulos/tinyply/blob/master/source/example.cpp
...
Result<std::unique_ptr<Scene>> Scene::from_ply(const std::string& file_name) {
    const double time = program_time();
    DEFER(std::cout << file_name << " loaded in " << std::round((program_time() - time) * 100.0) / 100.0 << "s" << std::endl);

    read_ply_file(file_name);
}
